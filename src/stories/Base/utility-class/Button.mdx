import { Canvas, Meta } from '@storybook/addon-docs/blocks'
import * as Stories from './button-class/button.stories'

<Meta of={Stories} />

# Button class

## Default button

```html
<button class="btn">Button</button>
```

## Button color class

```html
<button class="btn btn-secondary">Button</button>
<button class="btn btn-warning">Button</button>
```

## Button size

```html
<button class="btn btn-sm">Button</button>
<button class="btn btn-lg">Button</button>
```

## Button with round corners

```html
<button class="btn btn-round">Button</button>
```

## Button with full width of container

```html
<button class="btn btn-full">Button</button>
```

## Inverted button

```html
<button class="btn btn-invert">Button</button>
```

## Button group

```html
<div class="btn-group">
  <button class="btn">Button 1</button>
  <button class="btn btn-secondary">Button 2</button>
  <button class="btn btn-warning">Button 3</button>
  <button class="btn btn-invert btn-danger">Button 4</button>
</div>
```

## Button group span

`btn-group-span` class enables button group to use full width of parent and child buttons with equal width.

```html
<div class="btn-group btn-group-span">
  <button class="btn">Button 1</button>
  <button class="btn btn-secondary">Button 2</button>
  <button class="btn btn-warning">Button 3</button>
  <button class="btn btn-invert btn-danger">Button 4</button>
</div>
```

## Button progress animation

You need to nest text inside a tag such as div, span or text to make text visible on hover|focus.

```html
<button class="btn btn-progress">
  <span>Default</span>
</button>
<button class="btn btn-progress btn-secondary">
  <span>Secondary</span>
</button>
<button class="btn btn-full btn-progress btn-round btn-info">
  <span>Rounded</span>
</button>
<button class="btn btn-full btn-progress btn-progress-r btn-success">
  <span>Reverse</span>
</button>
<button class="btn btn-progress btn-full btn-warning">
  <span>Full width</span>
</button>
<button class="btn btn-progress btn-round btn-full btn-danger">
  <span>Full width rounded</span>
</button>
```

## Button with no ring on focus

```html
<button class="btn btn-no-ring">Button</button>
```

## Example

<Canvas of={Stories.buttonExample} />
