import { cva } from 'class-variance-authority'
import type { OpacityRange, Range } from './static/theme.types'

// --- color variants ---

export const bgColorVariant = cva('', {
  variants: {
    color: {
      primary: 'mcl-bg-primary',
      secondary: 'mcl-bg-secondary',
      success: 'mcl-bg-success',
      danger: 'mcl-bg-danger',
      info: 'mcl-bg-info',
      warning: 'mcl-bg-warning',
      'light-1': 'mcl-bg-light-1',
      'light-2': 'mcl-bg-light-2',
      'light-3': 'mcl-bg-light-3',
      'light-4': 'mcl-bg-light-4',
      'dark-1': 'mcl-bg-dark-1',
      'dark-2': 'mcl-bg-dark-2',
      'dark-3': 'mcl-bg-dark-3',
      'dark-4': 'mcl-bg-dark-4',
      black: 'mcl-bg-black',
      white: 'mcl-bg-white',
      transparent: 'mcl-bg-transparent',
    },
  },
})

export const hoverBgColorVariant = cva('', {
  variants: {
    color: {
      primary: 'hover:bg-primary',
      secondary: 'hover:bg-secondary',
      success: 'hover:bg-success',
      danger: 'hover:bg-danger',
      info: 'hover:bg-info',
      warning: 'hover:bg-warning',
      'light-1': 'hover:bg-light-1',
      'light-2': 'hover:bg-light-2',
      'light-3': 'hover:bg-light-3',
      'light-4': 'hover:bg-light-4',
      'dark-1': 'hover:bg-dark-1',
      'dark-2': 'hover:bg-dark-2',
      'dark-3': 'hover:bg-dark-3',
      'dark-4': 'hover:bg-dark-4',
      black: 'hover:bg-black',
      white: 'hover:bg-white',
      transparent: 'hover:bg-transparent',
    },
  },
})

export const focusBgColorVariant = cva('', {
  variants: {
    color: {
      primary: 'focus:bg-primary',
      secondary: 'focus:bg-secondary',
      success: 'focus:bg-success',
      danger: 'focus:bg-danger',
      info: 'focus:bg-info',
      warning: 'focus:bg-warning',
      'light-1': 'focus:bg-light-1',
      'light-2': 'focus:bg-light-2',
      'light-3': 'focus:bg-light-3',
      'light-4': 'focus:bg-light-4',
      'dark-1': 'focus:bg-dark-1',
      'dark-2': 'focus:bg-dark-2',
      'dark-3': 'focus:bg-dark-3',
      'dark-4': 'focus:bg-dark-4',
      black: 'focus:bg-black',
      white: 'focus:bg-white',
      transparent: 'focus:bg-transparent',
    },
  },
})

export const beforeBgColorVariant = cva('', {
  variants: {
    color: {
      primary: 'before:bg-primary',
      secondary: 'before:bg-secondary',
      success: 'before:bg-success',
      danger: 'before:bg-danger',
      info: 'before:bg-info',
      warning: 'before:bg-warning',
      'light-1': 'before:bg-light-1',
      'light-2': 'before:bg-light-2',
      'light-3': 'before:bg-light-3',
      'light-4': 'before:bg-light-4',
      'dark-1': 'before:bg-dark-1',
      'dark-2': 'before:bg-dark-2',
      'dark-3': 'before:bg-dark-3',
      'dark-4': 'before:bg-dark-4',
      black: 'before:bg-black',
      white: 'before:bg-white',
      transparent: 'before:bg-transparent',
    },
  },
})

export const afterBgColorVariant = cva('', {
  variants: {
    color: {
      primary: 'after:bg-primary',
      secondary: 'after:bg-secondary',
      success: 'after:bg-success',
      danger: 'after:bg-danger',
      info: 'after:bg-info',
      warning: 'after:bg-warning',
      'light-1': 'after:bg-light-1',
      'light-2': 'after:bg-light-2',
      'light-3': 'after:bg-light-3',
      'light-4': 'after:bg-light-4',
      'dark-1': 'after:bg-dark-1',
      'dark-2': 'after:bg-dark-2',
      'dark-3': 'after:bg-dark-3',
      'dark-4': 'after:bg-dark-4',
      black: 'after:bg-black',
      white: 'after:bg-white',
      transparent: 'after:bg-transparent',
    },
  },
})

export const activeBgColorVariant = cva('', {
  variants: {
    color: {
      primary: 'active:bg-primary',
      secondary: 'active:bg-secondary',
      success: 'active:bg-success',
      danger: 'active:bg-danger',
      info: 'active:bg-info',
      warning: 'active:bg-warning',
      'light-1': 'active:bg-light-1',
      'light-2': 'active:bg-light-2',
      'light-3': 'active:bg-light-3',
      'light-4': 'active:bg-light-4',
      'dark-1': 'active:bg-dark-1',
      'dark-2': 'active:bg-dark-2',
      'dark-3': 'active:bg-dark-3',
      'dark-4': 'active:bg-dark-4',
      black: 'active:bg-black',
      white: 'active:bg-white',
      transparent: 'active:bg-transparent',
    },
  },
})

export const textColorVariant = cva('', {
  variants: {
    color: {
      primary: 'text-primary',
      secondary: 'text-secondary',
      success: 'text-success',
      danger: 'text-danger',
      info: 'text-info',
      warning: 'text-warning',
      'light-1': 'text-light-1',
      'light-2': 'text-light-2',
      'light-3': 'text-light-3',
      'light-4': 'text-light-4',
      'dark-1': 'text-dark-1',
      'dark-2': 'text-dark-2',
      'dark-3': 'text-dark-3',
      'dark-4': 'text-dark-4',
      black: 'text-black',
      white: 'text-white',
      transparent: 'text-transparent',
    },
  },
})

export const hoverTextColorVariant = cva('', {
  variants: {
    color: {
      primary: 'hover:text-primary',
      secondary: 'hover:text-secondary',
      success: 'hover:text-success',
      danger: 'hover:text-danger',
      info: 'hover:text-info',
      warning: 'hover:text-warning',
      'light-1': 'hover:text-light-1',
      'light-2': 'hover:text-light-2',
      'light-3': 'hover:text-light-3',
      'light-4': 'hover:text-light-4',
      'dark-1': 'hover:text-dark-1',
      'dark-2': 'hover:text-dark-2',
      'dark-3': 'hover:text-dark-3',
      'dark-4': 'hover:text-dark-4',
      black: 'hover:text-black',
      white: 'hover:text-white',
      transparent: 'hover:text-transparent',
    },
  },
})

export const focusTextColorVariant = cva('', {
  variants: {
    color: {
      primary: 'focus:text-primary',
      secondary: 'focus:text-secondary',
      success: 'focus:text-success',
      danger: 'focus:text-danger',
      info: 'focus:text-info',
      warning: 'focus:text-warning',
      'light-1': 'focus:text-light-1',
      'light-2': 'focus:text-light-2',
      'light-3': 'focus:text-light-3',
      'light-4': 'focus:text-light-4',
      'dark-1': 'focus:text-dark-1',
      'dark-2': 'focus:text-dark-2',
      'dark-3': 'focus:text-dark-3',
      'dark-4': 'focus:text-dark-4',
      black: 'focus:text-black',
      white: 'focus:text-white',
      transparent: 'focus:text-transparent',
    },
  },
})

export const disabledTextColorVariant = cva('', {
  variants: {
    color: {
      primary: 'disabled:text-primary',
      secondary: 'disabled:text-secondary',
      success: 'disabled:text-success',
      danger: 'disabled:text-danger',
      info: 'disabled:text-info',
      warning: 'disabled:text-warning',
      'light-1': 'disabled:text-light-1',
      'light-2': 'disabled:text-light-2',
      'light-3': 'disabled:text-light-3',
      'light-4': 'disabled:text-light-4',
      'dark-1': 'disabled:text-dark-1',
      'dark-2': 'disabled:text-dark-2',
      'dark-3': 'disabled:text-dark-3',
      'dark-4': 'disabled:text-dark-4',
      black: 'disabled:text-black',
      white: 'disabled:text-white',
      transparent: 'disabled:text-transparent',
    },
  },
})

export const svgFillColorVariant = cva('', {
  variants: {
    color: {
      primary: 'fill-primary',
      secondary: 'fill-secondary',
      success: 'fill-success',
      danger: 'fill-danger',
      info: 'fill-info',
      warning: 'fill-warning',
      'light-1': 'fill-light-1',
      'light-2': 'fill-light-2',
      'light-3': 'fill-light-3',
      'light-4': 'fill-light-4',
      'dark-1': 'fill-dark-1',
      'dark-2': 'fill-dark-2',
      'dark-3': 'fill-dark-3',
      'dark-4': 'fill-dark-4',
      black: 'fill-black',
      white: 'fill-white',
      transparent: 'fill-transparent',
    },
  },
})

export const ringColorVariant = cva('', {
  variants: {
    color: {
      primary: 'ring-primary',
      secondary: 'ring-secondary',
      success: 'ring-success',
      danger: 'ring-danger',
      info: 'ring-info',
      warning: 'ring-warning',
      'light-1': 'ring-light-1',
      'light-2': 'ring-light-2',
      'light-3': 'ring-light-3',
      'light-4': 'ring-light-4',
      'dark-1': 'ring-dark-1',
      'dark-2': 'ring-dark-2',
      'dark-3': 'ring-dark-3',
      'dark-4': 'ring-dark-4',
      black: 'ring-black',
      white: 'ring-white',
      transparent: 'ring-transparent',
    },
  },
})

export const focusRingColorVariant = cva('', {
  variants: {
    color: {
      primary: 'focus:ring-primary',
      secondary: 'focus:ring-secondary',
      success: 'focus:ring-success',
      danger: 'focus:ring-danger',
      info: 'focus:ring-info',
      warning: 'focus:ring-warning',
      'light-1': 'focus:ring-light-1',
      'light-2': 'focus:ring-light-2',
      'light-3': 'focus:ring-light-3',
      'light-4': 'focus:ring-light-4',
      'dark-1': 'focus:ring-dark-1',
      'dark-2': 'focus:ring-dark-2',
      'dark-3': 'focus:ring-dark-3',
      'dark-4': 'focus:ring-dark-4',
      black: 'focus:ring-black',
      white: 'focus:ring-white',
      transparent: 'focus:ring-transparent',
    },
  },
})

export const activeRingColorVariant = cva('', {
  variants: {
    color: {
      primary: 'active:ring-primary',
      secondary: 'active:ring-secondary',
      success: 'active:ring-success',
      danger: 'active:ring-danger',
      info: 'active:ring-info',
      warning: 'active:ring-warning',
      'light-1': 'active:ring-light-1',
      'light-2': 'active:ring-light-2',
      'light-3': 'active:ring-light-3',
      'light-4': 'active:ring-light-4',
      'dark-1': 'active:ring-dark-1',
      'dark-2': 'active:ring-dark-2',
      'dark-3': 'active:ring-dark-3',
      'dark-4': 'active:ring-dark-4',
      black: 'active:ring-black',
      white: 'active:ring-white',
      transparent: 'active:ring-transparent',
    },
  },
})

export const ringOffsetColorVariant = cva('', {
  variants: {
    color: {
      primary: 'ring-offset-primary',
      secondary: 'ring-offset-secondary',
      success: 'ring-offset-success',
      danger: 'ring-offset-danger',
      info: 'ring-offset-info',
      warning: 'ring-offset-warning',
      'light-1': 'ring-offset-light-1',
      'light-2': 'ring-offset-light-2',
      'light-3': 'ring-offset-light-3',
      'light-4': 'ring-offset-light-4',
      'dark-1': 'ring-offset-dark-1',
      'dark-2': 'ring-offset-dark-2',
      'dark-3': 'ring-offset-dark-3',
      'dark-4': 'ring-offset-dark-4',
      black: 'ring-offset-black',
      white: 'ring-offset-white',
      transparent: 'ring-offset-transparent',
    },
  },
})

export const outlineColorVariant = cva('', {
  variants: {
    color: {
      primary: 'mcl-outline-primary',
      secondary: 'mcl-outline-secondary',
      success: 'mcl-outline-success',
      danger: 'mcl-outline-danger',
      info: 'mcl-outline-info',
      warning: 'mcl-outline-warning',
      'light-1': 'mcl-outline-light-1',
      'light-2': 'mcl-outline-light-2',
      'light-3': 'mcl-outline-light-3',
      'light-4': 'mcl-outline-light-4',
      'dark-1': 'mcl-outline-dark-1',
      'dark-2': 'mcl-outline-dark-2',
      'dark-3': 'mcl-outline-dark-3',
      'dark-4': 'mcl-outline-dark-4',
      black: 'mcl-outline-black',
      white: 'mcl-outline-white',
      transparent: 'mcl-outline-transparent',
    },
  },
})

export const focusOutlineColorVariant = cva('', {
  variants: {
    color: {
      primary: 'focus:mcl-outline-primary',
      secondary: 'focus:mcl-outline-secondary',
      success: 'focus:mcl-outline-success',
      danger: 'focus:mcl-outline-danger',
      info: 'focus:mcl-outline-info',
      warning: 'focus:mcl-outline-warning',
      'light-1': 'focus:mcl-outline-light-1',
      'light-2': 'focus:mcl-outline-light-2',
      'light-3': 'focus:mcl-outline-light-3',
      'light-4': 'focus:mcl-outline-light-4',
      'dark-1': 'focus:mcl-outline-dark-1',
      'dark-2': 'focus:mcl-outline-dark-2',
      'dark-3': 'focus:mcl-outline-dark-3',
      'dark-4': 'focus:mcl-outline-dark-4',
      black: 'focus:mcl-outline-black',
      white: 'focus:mcl-outline-white',
      transparent: 'focus:mcl-outline-transparent',
    },
  },
})

export const activeOutlineColorVariant = cva('', {
  variants: {
    color: {
      primary: 'active:mcl-outline-primary',
      secondary: 'active:mcl-outline-secondary',
      success: 'active:mcl-outline-success',
      danger: 'active:mcl-outline-danger',
      info: 'active:mcl-outline-info',
      warning: 'active:mcl-outline-warning',
      'light-1': 'active:mcl-outline-light-1',
      'light-2': 'active:mcl-outline-light-2',
      'light-3': 'active:mcl-outline-light-3',
      'light-4': 'active:mcl-outline-light-4',
      'dark-1': 'active:mcl-outline-dark-1',
      'dark-2': 'active:mcl-outline-dark-2',
      'dark-3': 'active:mcl-outline-dark-3',
      'dark-4': 'active:mcl-outline-dark-4',
      black: 'active:mcl-outline-black',
      white: 'active:mcl-outline-white',
      transparent: 'active:mcl-outline-transparent',
    },
  },
})

export const btnColorVariant = cva('', {
  variants: {
    color: {
      primary: 'btn-primary',
      secondary: 'btn-secondary',
      success: 'btn-success',
      danger: 'btn-danger',
      info: 'btn-info',
      warning: 'btn-warning',
      'light-1': 'btn-light-1',
      'light-2': 'btn-light-2',
      'light-3': 'btn-light-3',
      'light-4': 'btn-light-4',
      'dark-1': 'btn-dark-1',
      'dark-2': 'btn-dark-2',
      'dark-3': 'btn-dark-3',
      'dark-4': 'btn-dark-4',
      black: 'btn-black',
      white: 'btn-white',
      transparent: 'btn-transparent',
    },
  },
})

export const listColorVariant = cva('', {
  variants: {
    color: {
      primary: 'mcl-list-primary',
      secondary: 'mcl-list-secondary',
      success: 'mcl-list-success',
      danger: 'mcl-list-danger',
      info: 'mcl-list-info',
      warning: 'mcl-list-warning',
      'light-1': 'mcl-list-light-1',
      'light-2': 'mcl-list-light-2',
      'light-3': 'mcl-list-light-3',
      'light-4': 'mcl-list-light-4',
      'dark-1': 'mcl-list-dark-1',
      'dark-2': 'mcl-list-dark-2',
      'dark-3': 'mcl-list-dark-3',
      'dark-4': 'mcl-list-dark-4',
      black: 'mcl-list-black',
      white: 'mcl-list-white',
      transparent: 'mcl-list-transparent',
    },
  },
})

export const borderColorVariant = cva('', {
  variants: {
    color: {
      primary: 'border-primary',
      secondary: 'border-secondary',
      success: 'border-success',
      danger: 'border-danger',
      info: 'border-info',
      warning: 'border-warning',
      'light-1': 'border-light-1',
      'light-2': 'border-light-2',
      'light-3': 'border-light-3',
      'light-4': 'border-light-4',
      'dark-1': 'border-dark-1',
      'dark-2': 'border-dark-2',
      'dark-3': 'border-dark-3',
      'dark-4': 'border-dark-4',
      black: 'border-black',
      white: 'border-white',
      transparent: 'border-transparent',
    },
  },
})

export const borderXColorVariant = cva('', {
  variants: {
    color: {
      primary: 'border-x-primary',
      secondary: 'border-x-secondary',
      success: 'border-x-success',
      danger: 'border-x-danger',
      info: 'border-x-info',
      warning: 'border-x-warning',
      'light-1': 'border-x-light-1',
      'light-2': 'border-x-light-2',
      'light-3': 'border-x-light-3',
      'light-4': 'border-x-light-4',
      'dark-1': 'border-x-dark-1',
      'dark-2': 'border-x-dark-2',
      'dark-3': 'border-x-dark-3',
      'dark-4': 'border-x-dark-4',
      black: 'border-x-black',
      white: 'border-x-white',
      transparent: 'border-x-transparent',
    },
  },
})

export const borderYColorVariant = cva('', {
  variants: {
    color: {
      primary: 'border-y-primary',
      secondary: 'border-y-secondary',
      success: 'border-y-success',
      danger: 'border-y-danger',
      info: 'border-y-info',
      warning: 'border-y-warning',
      'light-1': 'border-y-light-1',
      'light-2': 'border-y-light-2',
      'light-3': 'border-y-light-3',
      'light-4': 'border-y-light-4',
      'dark-1': 'border-y-dark-1',
      'dark-2': 'border-y-dark-2',
      'dark-3': 'border-y-dark-3',
      'dark-4': 'border-y-dark-4',
      black: 'border-y-black',
      white: 'border-y-white',
      transparent: 'border-y-transparent',
    },
  },
})

export const borderTopColorVariant = cva('', {
  variants: {
    color: {
      primary: 'border-t-primary',
      secondary: 'border-t-secondary',
      success: 'border-t-success',
      danger: 'border-t-danger',
      info: 'border-t-info',
      warning: 'border-t-warning',
      'light-1': 'border-t-light-1',
      'light-2': 'border-t-light-2',
      'light-3': 'border-t-light-3',
      'light-4': 'border-t-light-4',
      'dark-1': 'border-t-dark-1',
      'dark-2': 'border-t-dark-2',
      'dark-3': 'border-t-dark-3',
      'dark-4': 'border-t-dark-4',
      black: 'border-t-black',
      white: 'border-t-white',
      transparent: 'border-t-transparent',
    },
  },
})

export const borderBottomColorVariant = cva('', {
  variants: {
    color: {
      primary: 'border-b-primary',
      secondary: 'border-b-secondary',
      success: 'border-b-success',
      danger: 'border-b-danger',
      info: 'border-b-info',
      warning: 'border-b-warning',
      'light-1': 'border-b-light-1',
      'light-2': 'border-b-light-2',
      'light-3': 'border-b-light-3',
      'light-4': 'border-b-light-4',
      'dark-1': 'border-b-dark-1',
      'dark-2': 'border-b-dark-2',
      'dark-3': 'border-b-dark-3',
      'dark-4': 'border-b-dark-4',
      black: 'border-b-black',
      white: 'border-b-white',
      transparent: 'border-b-transparent',
    },
  },
})

export const borderLeftColorVariant = cva('', {
  variants: {
    color: {
      primary: 'border-l-primary',
      secondary: 'border-l-secondary',
      success: 'border-l-success',
      danger: 'border-l-danger',
      info: 'border-l-info',
      warning: 'border-l-warning',
      'light-1': 'border-l-light-1',
      'light-2': 'border-l-light-2',
      'light-3': 'border-l-light-3',
      'light-4': 'border-l-light-4',
      'dark-1': 'border-l-dark-1',
      'dark-2': 'border-l-dark-2',
      'dark-3': 'border-l-dark-3',
      'dark-4': 'border-l-dark-4',
      black: 'border-l-black',
      white: 'border-l-white',
      transparent: 'border-l-transparent',
    },
  },
})

export const borderRightColorVariant = cva('', {
  variants: {
    color: {
      primary: 'border-r-primary',
      secondary: 'border-r-secondary',
      success: 'border-r-success',
      danger: 'border-r-danger',
      info: 'border-r-info',
      warning: 'border-r-warning',
      'light-1': 'border-r-light-1',
      'light-2': 'border-r-light-2',
      'light-3': 'border-r-light-3',
      'light-4': 'border-r-light-4',
      'dark-1': 'border-r-dark-1',
      'dark-2': 'border-r-dark-2',
      'dark-3': 'border-r-dark-3',
      'dark-4': 'border-r-dark-4',
      black: 'border-r-black',
      white: 'border-r-white',
      transparent: 'border-r-transparent',
    },
  },
})

// --- typography variants ---

export const bodyTextVariant = cva('', {
  variants: {
    size: {
      xs: 'text-xs',
      sm: 'text-sm',
      md: 'text-md',
      lg: 'text-lg',
      xl: 'text-xl',
    },
  },
})

export const fontWeightVariant = cva('', {
  variants: {
    weight: {
      light: 'font-light',
      normal: 'font-normal',
      bold: 'font-bold',
    },
  },
})

export const textAlignVariant = cva('', {
  variants: {
    align: {
      left: 'text-left',
      center: 'text-center',
      right: 'text-right',
    },
  },
})

export const h1Variant = cva('', {
  variants: {
    size: {
      sm: 'h1-sm',
      md: 'h1-md',
      lg: 'h1-lg',
      xl: 'h1-xl',
    },
  },
})

export const h2Variant = cva('', {
  variants: {
    size: {
      sm: 'h2-sm',
      md: 'h2-md',
      lg: 'h2-lg',
      xl: 'h2-xl',
    },
  },
})

export const h3Variant = cva('', {
  variants: {
    size: {
      sm: 'h3-sm',
      md: 'h3-md',
      lg: 'h3-lg',
      xl: 'h3-xl',
    },
  },
})

export const h4Variant = cva('', {
  variants: {
    size: {
      sm: 'h4-sm',
      md: 'h4-md',
      lg: 'h4-lg',
      xl: 'h4-xl',
    },
  },
})

// --- border width variants (numeric keys — plain map, same call interface as cva) ---

const _borderWidthMap: Record<Range<1, 12>, string> = {
  1: 'border-[1px]',
  2: 'border-[2px]',
  3: 'border-[3px]',
  4: 'border-[4px]',
  5: 'border-[5px]',
  6: 'border-[6px]',
  7: 'border-[7px]',
  8: 'border-[8px]',
  9: 'border-[9px]',
  10: 'border-[10px]',
  11: 'border-[11px]',
  12: 'border-[12px]',
}
const _borderXWidthMap: Record<Range<1, 12>, string> = {
  1: 'border-x-[1px]',
  2: 'border-x-[2px]',
  3: 'border-x-[3px]',
  4: 'border-x-[4px]',
  5: 'border-x-[5px]',
  6: 'border-x-[6px]',
  7: 'border-x-[7px]',
  8: 'border-x-[8px]',
  9: 'border-x-[9px]',
  10: 'border-x-[10px]',
  11: 'border-x-[11px]',
  12: 'border-x-[12px]',
}
const _borderYWidthMap: Record<Range<1, 12>, string> = {
  1: 'border-y-[1px]',
  2: 'border-y-[2px]',
  3: 'border-y-[3px]',
  4: 'border-y-[4px]',
  5: 'border-y-[5px]',
  6: 'border-y-[6px]',
  7: 'border-y-[7px]',
  8: 'border-y-[8px]',
  9: 'border-y-[9px]',
  10: 'border-y-[10px]',
  11: 'border-y-[11px]',
  12: 'border-y-[12px]',
}
const _borderTopWidthMap: Record<Range<1, 12>, string> = {
  1: 'border-t-[1px]',
  2: 'border-t-[2px]',
  3: 'border-t-[3px]',
  4: 'border-t-[4px]',
  5: 'border-t-[5px]',
  6: 'border-t-[6px]',
  7: 'border-t-[7px]',
  8: 'border-t-[8px]',
  9: 'border-t-[9px]',
  10: 'border-t-[10px]',
  11: 'border-t-[11px]',
  12: 'border-t-[12px]',
}
const _borderBottomWidthMap: Record<Range<1, 12>, string> = {
  1: 'border-b-[1px]',
  2: 'border-b-[2px]',
  3: 'border-b-[3px]',
  4: 'border-b-[4px]',
  5: 'border-b-[5px]',
  6: 'border-b-[6px]',
  7: 'border-b-[7px]',
  8: 'border-b-[8px]',
  9: 'border-b-[9px]',
  10: 'border-b-[10px]',
  11: 'border-b-[11px]',
  12: 'border-b-[12px]',
}
const _borderLeftWidthMap: Record<Range<1, 12>, string> = {
  1: 'border-l-[1px]',
  2: 'border-l-[2px]',
  3: 'border-l-[3px]',
  4: 'border-l-[4px]',
  5: 'border-l-[5px]',
  6: 'border-l-[6px]',
  7: 'border-l-[7px]',
  8: 'border-l-[8px]',
  9: 'border-l-[9px]',
  10: 'border-l-[10px]',
  11: 'border-l-[11px]',
  12: 'border-l-[12px]',
}
const _borderRightWidthMap: Record<Range<1, 12>, string> = {
  1: 'border-r-[1px]',
  2: 'border-r-[2px]',
  3: 'border-r-[3px]',
  4: 'border-r-[4px]',
  5: 'border-r-[5px]',
  6: 'border-r-[6px]',
  7: 'border-r-[7px]',
  8: 'border-r-[8px]',
  9: 'border-r-[9px]',
  10: 'border-r-[10px]',
  11: 'border-r-[11px]',
  12: 'border-r-[12px]',
}

export const borderWidthVariant = (p: { width: Range<1, 12> }): string =>
  _borderWidthMap[p.width] ?? ''
export const borderXWidthVariant = (p: { width: Range<1, 12> }): string =>
  _borderXWidthMap[p.width] ?? ''
export const borderYWidthVariant = (p: { width: Range<1, 12> }): string =>
  _borderYWidthMap[p.width] ?? ''
export const borderTopWidthVariant = (p: { width: Range<1, 12> }): string =>
  _borderTopWidthMap[p.width] ?? ''
export const borderBottomWidthVariant = (p: { width: Range<1, 12> }): string =>
  _borderBottomWidthMap[p.width] ?? ''
export const borderLeftWidthVariant = (p: { width: Range<1, 12> }): string =>
  _borderLeftWidthMap[p.width] ?? ''
export const borderRightWidthVariant = (p: { width: Range<1, 12> }): string =>
  _borderRightWidthMap[p.width] ?? ''

// --- opacity variants (numeric keys — plain map, same call interface as cva) ---

const _opacityMap: Record<OpacityRange, string> = {
  10: 'opacity-10',
  20: 'opacity-20',
  30: 'opacity-30',
  40: 'opacity-40',
  50: 'opacity-50',
  60: 'opacity-60',
  70: 'opacity-70',
  80: 'opacity-80',
  90: 'opacity-90',
  100: 'opacity-100',
}
const _bgOpacityMap: Record<OpacityRange, string> = {
  10: 'bg-opacity-10',
  20: 'bg-opacity-20',
  30: 'bg-opacity-30',
  40: 'bg-opacity-40',
  50: 'bg-opacity-50',
  60: 'bg-opacity-60',
  70: 'bg-opacity-70',
  80: 'bg-opacity-80',
  90: 'bg-opacity-90',
  100: 'bg-opacity-100',
}
const _outlineOpacityMap: Record<OpacityRange, string> = {
  10: 'outline-opacity-10',
  20: 'outline-opacity-20',
  30: 'outline-opacity-30',
  40: 'outline-opacity-40',
  50: 'outline-opacity-50',
  60: 'outline-opacity-60',
  70: 'outline-opacity-70',
  80: 'outline-opacity-80',
  90: 'outline-opacity-90',
  100: 'outline-opacity-100',
}

export const opacityVariant = (p: { opacity: OpacityRange }): string =>
  _opacityMap[p.opacity] ?? ''
export const bgOpacityVariant = (p: { opacity: OpacityRange }): string =>
  _bgOpacityMap[p.opacity] ?? ''
export const outlineOpacityVariant = (p: { opacity: OpacityRange }): string =>
  _outlineOpacityMap[p.opacity] ?? ''

// --- spacing variants ---

export const marginVariant = cva('', {
  variants: {
    spacing: {
      '0': 'm-0',
      '3xs': 'm-3xs',
      '2xs': 'm-2xs',
      xs: 'm-xs',
      sm: 'm-sm',
      md: 'm-md',
      lg: 'm-lg',
      xl: 'm-xl',
      '2xl': 'm-2xl',
      '3xl': 'm-3xl',
    },
  },
})

export const marginXVariant = cva('', {
  variants: {
    spacing: {
      '0': 'mx-0',
      '3xs': 'mx-3xs',
      '2xs': 'mx-2xs',
      xs: 'mx-xs',
      sm: 'mx-sm',
      md: 'mx-md',
      lg: 'mx-lg',
      xl: 'mx-xl',
      '2xl': 'mx-2xl',
      '3xl': 'mx-3xl',
    },
  },
})

export const marginYVariant = cva('', {
  variants: {
    spacing: {
      '0': 'my-0',
      '3xs': 'my-3xs',
      '2xs': 'my-2xs',
      xs: 'my-xs',
      sm: 'my-sm',
      md: 'my-md',
      lg: 'my-lg',
      xl: 'my-xl',
      '2xl': 'my-2xl',
      '3xl': 'my-3xl',
    },
  },
})

export const marginTopVariant = cva('', {
  variants: {
    spacing: {
      '0': 'mt-0',
      '3xs': 'mt-3xs',
      '2xs': 'mt-2xs',
      xs: 'mt-xs',
      sm: 'mt-sm',
      md: 'mt-md',
      lg: 'mt-lg',
      xl: 'mt-xl',
      '2xl': 'mt-2xl',
      '3xl': 'mt-3xl',
    },
  },
})

export const marginBottomVariant = cva('', {
  variants: {
    spacing: {
      '0': 'mb-0',
      '3xs': 'mb-3xs',
      '2xs': 'mb-2xs',
      xs: 'mb-xs',
      sm: 'mb-sm',
      md: 'mb-md',
      lg: 'mb-lg',
      xl: 'mb-xl',
      '2xl': 'mb-2xl',
      '3xl': 'mb-3xl',
    },
  },
})

export const marginLeftVariant = cva('', {
  variants: {
    spacing: {
      '0': 'ml-0',
      '3xs': 'ml-3xs',
      '2xs': 'ml-2xs',
      xs: 'ml-xs',
      sm: 'ml-sm',
      md: 'ml-md',
      lg: 'ml-lg',
      xl: 'ml-xl',
      '2xl': 'ml-2xl',
      '3xl': 'ml-3xl',
    },
  },
})

export const marginRightVariant = cva('', {
  variants: {
    spacing: {
      '0': 'mr-0',
      '3xs': 'mr-3xs',
      '2xs': 'mr-2xs',
      xs: 'mr-xs',
      sm: 'mr-sm',
      md: 'mr-md',
      lg: 'mr-lg',
      xl: 'mr-xl',
      '2xl': 'mr-2xl',
      '3xl': 'mr-3xl',
    },
  },
})

export const paddingVariant = cva('', {
  variants: {
    spacing: {
      '0': 'p-0',
      '3xs': 'p-3xs',
      '2xs': 'p-2xs',
      xs: 'p-xs',
      sm: 'p-sm',
      md: 'p-md',
      lg: 'p-lg',
      xl: 'p-xl',
      '2xl': 'p-2xl',
      '3xl': 'p-3xl',
    },
  },
})

export const paddingXVariant = cva('', {
  variants: {
    spacing: {
      '0': 'px-0',
      '3xs': 'px-3xs',
      '2xs': 'px-2xs',
      xs: 'px-xs',
      sm: 'px-sm',
      md: 'px-md',
      lg: 'px-lg',
      xl: 'px-xl',
      '2xl': 'px-2xl',
      '3xl': 'px-3xl',
    },
  },
})

export const paddingYVariant = cva('', {
  variants: {
    spacing: {
      '0': 'py-0',
      '3xs': 'py-3xs',
      '2xs': 'py-2xs',
      xs: 'py-xs',
      sm: 'py-sm',
      md: 'py-md',
      lg: 'py-lg',
      xl: 'py-xl',
      '2xl': 'py-2xl',
      '3xl': 'py-3xl',
    },
  },
})

export const paddingTopVariant = cva('', {
  variants: {
    spacing: {
      '0': 'pt-0',
      '3xs': 'pt-3xs',
      '2xs': 'pt-2xs',
      xs: 'pt-xs',
      sm: 'pt-sm',
      md: 'pt-md',
      lg: 'pt-lg',
      xl: 'pt-xl',
      '2xl': 'pt-2xl',
      '3xl': 'pt-3xl',
    },
  },
})

export const paddingBottomVariant = cva('', {
  variants: {
    spacing: {
      '0': 'pb-0',
      '3xs': 'pb-3xs',
      '2xs': 'pb-2xs',
      xs: 'pb-xs',
      sm: 'pb-sm',
      md: 'pb-md',
      lg: 'pb-lg',
      xl: 'pb-xl',
      '2xl': 'pb-2xl',
      '3xl': 'pb-3xl',
    },
  },
})

export const paddingLeftVariant = cva('', {
  variants: {
    spacing: {
      '0': 'pl-0',
      '3xs': 'pl-3xs',
      '2xs': 'pl-2xs',
      xs: 'pl-xs',
      sm: 'pl-sm',
      md: 'pl-md',
      lg: 'pl-lg',
      xl: 'pl-xl',
      '2xl': 'pl-2xl',
      '3xl': 'pl-3xl',
    },
  },
})

export const paddingRightVariant = cva('', {
  variants: {
    spacing: {
      '0': 'pr-0',
      '3xs': 'pr-3xs',
      '2xs': 'pr-2xs',
      xs: 'pr-xs',
      sm: 'pr-sm',
      md: 'pr-md',
      lg: 'pr-lg',
      xl: 'pr-xl',
      '2xl': 'pr-2xl',
      '3xl': 'pr-3xl',
    },
  },
})

export const gapVariant = cva('', {
  variants: {
    spacing: {
      '0': 'gap-0',
      '3xs': 'gap-3xs',
      '2xs': 'gap-2xs',
      xs: 'gap-xs',
      sm: 'gap-sm',
      md: 'gap-md',
      lg: 'gap-lg',
      xl: 'gap-xl',
      '2xl': 'gap-2xl',
      '3xl': 'gap-3xl',
    },
  },
})

// --- namespace object (named export — the primary public API) ---

export const generateClass = {
  // color
  bgColorVariant,
  hoverBgColorVariant,
  focusBgColorVariant,
  beforeBgColorVariant,
  afterBgColorVariant,
  activeBgColorVariant,
  textColorVariant,
  hoverTextColorVariant,
  focusTextColorVariant,
  disabledTextColorVariant,
  svgFillColorVariant,
  ringColorVariant,
  focusRingColorVariant,
  activeRingColorVariant,
  ringOffsetColorVariant,
  outlineColorVariant,
  focusOutlineColorVariant,
  activeOutlineColorVariant,
  btnColorVariant,
  listColorVariant,
  borderColorVariant,
  borderXColorVariant,
  borderYColorVariant,
  borderTopColorVariant,
  borderBottomColorVariant,
  borderLeftColorVariant,
  borderRightColorVariant,
  // typography
  bodyTextVariant,
  fontWeightVariant,
  textAlignVariant,
  h1Variant,
  h2Variant,
  h3Variant,
  h4Variant,
  // border width
  borderWidthVariant,
  borderXWidthVariant,
  borderYWidthVariant,
  borderTopWidthVariant,
  borderBottomWidthVariant,
  borderLeftWidthVariant,
  borderRightWidthVariant,
  // opacity
  opacityVariant,
  bgOpacityVariant,
  outlineOpacityVariant,
  // spacing
  marginVariant,
  marginXVariant,
  marginYVariant,
  marginTopVariant,
  marginBottomVariant,
  marginLeftVariant,
  marginRightVariant,
  paddingVariant,
  paddingXVariant,
  paddingYVariant,
  paddingTopVariant,
  paddingBottomVariant,
  paddingLeftVariant,
  paddingRightVariant,
  gapVariant,
}

